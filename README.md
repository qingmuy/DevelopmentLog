...
![img](./assets/0da0f634e52a4683b6d615861c908605.jpeg)
...
- 即便是消费者和提供者双方使用了相同的依赖也可能存在依赖不一致的情况，具体参考：
  ...
  ![报错信息1](./assets/QQ图片20240913230514.png)

  ![报错图片2](./assets/QQ图片20240913230522.png)
...
![img](./assets/H7%%TD(KD{0%{O3D`L_O)%5.png)
...
![image-20241010190443871](./assets/image-20241010190443871.png)
...
对于**缓存穿透**问题，其出现的场景为：若某请求查询一根本不存在的数据，此时该数据在 Redis 中无法查询到，后端便会访问数据库并返回结果，但是此情况下数据库中也无内容，响应会直接返回，而并不更改缓存中的任何内容。但若再发起一次这样的请求，后端还算是会查询数据库，这就会造成漏洞，可能会被攻击。
...
对于**缓存击穿**的问题：实际上就是在多线程高并发的情况下，某一数据在缓存中失效 (通常为热点数据)，而此时有大量的数据访问，则会直接查询数据库，可能对服务造成影响。

其解决方法在于：一是直接设置热点数据永不过期，并通过定时任务更新数据；二使用分布式锁，仅允许一个线程访问数据库并写入缓存；三是加互斥锁，若线程获取互斥锁则开一个新线程去处理数据更新操作，其他线程未获取互斥锁则直接返回脏数据。

第一个方法的问题在于可能会有段时间读取到脏数据

第二个方法需要用过分布式锁(即互斥锁)实现，一定程度上影响性能 

第三个方法不影响性能，但是无法保证数据一致性

本质上还是CAP定理，即在可用性和一致性之间做出抉择。



对于**缓存雪崩**问题：就是缓存击穿问题的升级，即在同一时间内，多个缓存同时失效，均需要访问数据库进行缓存的更新，此时便会造成比缓存击穿更为严重的性能堵塞甚至宕机。
...
[![1653374296906](./assets/202303090033838.png)](https://markdown-1258124344.cos.ap-guangzhou.myqcloud.com/images/202303090033838.png)
...
通过其`getLock()`方法获取锁，即可避免锁的重入问题，其解决机制就是`使用hash结构存储锁`，对应键为锁的名称，field为线程的uuid，value即为有几把锁。
...
Kibana连接ES集群，只需指定一个ip即可：建议外网+端口



### Spring boot配置数据库

如果项目/模块内引入了Mybatis/Mybatis-plus，就需要配置数据库来源，否则会在启动时报错，解决方案有两种：

1. 避免引入Mybatis/Mybatis-plus的依赖，若是引入其他依赖时引入的配置排除即可。
